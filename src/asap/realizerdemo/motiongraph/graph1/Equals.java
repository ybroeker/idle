...
public class Equals implements IEquals {
    ...
    public boolean startEndEquals(SkeletonInterpolator start, SkeletonInterpolator end) {
        ...
        for (part = 0; part < end.getPartIds().length; part++) {
            endConfigMap.put(end.getPartIds()[part], new float[]{endConfig[i+part*3],endConfig[i+1+part*3],endConfig[i+2+part*3],endConfig[i+3+part*3]});
            startConfigMap.put(start.getPartIds()[part], new float[]{startConfig[i+part*3],startConfig[i+1+part*3],startConfig[i+2+part*3],startConfig[i+3+part*3]});
        }
